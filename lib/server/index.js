  ...
  utilities.duplicate(task.options, filePath, function(duplicate) {
    if (!duplicate) {
      queue.push({
        ...
        persistentPath: task.persistentPath,
        series: task.series
      });
    }
    done();
...
function enqueueChapterPersistent(emitter, task, chapter, done) {
  ...
  if (typeof task.persistent[identifier] !== 'undefined') {
    ...
    fs.rename(previousPath, currentPath, function(err) {
      if (err) return done(err);
...
function enqueueSeries(emitter, queue, task, done) {
  request(task.series, 'utf8', function(err) {
    ...
    task.persistentPath = filePath;
    fs.exists(filePath, function(exists) {
      if (!exists) {
        task.persistent = {};
        return enqueueSeriesNext(emitter, queue, task, title, done);
      }
      fs.readFile(filePath, 'utf8', function(err, contents) {
        if (err) return done(err);
        task.persistent = JSON.parse(contents);
        enqueueSeriesNext(emitter, queue, task, title, done);
      });
    });
  });
}
 ...
 * @param {function(Error)} done
 */
function enqueueSeriesNext(emitter, queue, task, title, done) {
  shared.common.async.eachSeries(task.series.children, function(chapter, next) {
    ...
    emitter.emit('data', {item: title, type: 'surveyed'});
    done(err);
  });
}
...
function save(filePath, record, done) {
  if (!Object.keys(record).length) return process.nextTick(done);
