 ...
 * @param {Meta=} meta
 */
function Agent(chapterPath, processors, meta) {
...
Agent.prototype.add = function(address, number, done) {
  ...
  request(address, 'binary', function(err, data) {
    ...
    processors(that, buffer, function(err, buffer) {
      if (err) return done(err);
      extension = detect(buffer);
...
Agent.prototype.publish = function(done) {
  if (!this._initialized) return done();
  if (this._meta) this._zip.append(this._meta.xml(), {name: 'ComicInfo.xml'});
  this._zip.finalize();
  if (this._disposed) return fs.unlink(this._chapterPath + suffix, done);
  fs.rename(this._chapterPath + suffix, this._chapterPath, done);
};
...
function initialize(that, done) {
  ...
  create(path.dirname(that._chapterPath), function() {
    that._zip.pipe(fs.createWriteStream(that._chapterPath + suffix));
 ...
 */
function processors(that, buffer, done) {
